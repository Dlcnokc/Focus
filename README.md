...
**Status (as of last session):** Core v1 loop works — create / edit / delete, drag move & reorder with live preview, save in the browser on refresh. Solo only. No deploy, export, or multi-user yet.
...
| Later | Deploy online (still solo) | e.g. static host + same localStorage limits unless backend |
...
| **Edit** | **Edit** on card → centered modal (title + notes); title required |
| **Delete** | **Delete** → themed confirm modal (blurred backdrop); no browser `alert` |
| **Drag** | Drag from **anywhere on the card** (not Edit/Delete buttons) |
...
3. Edit notes → Save  
4. Drag between columns; confirm live reordering  
5. Refresh page → board still there  
6. Delete with confirm → Cancel and confirm both work  
